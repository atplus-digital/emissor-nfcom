...
import type { GenerateTypesPipelineCtx } from "@pipelines/generate-types/stages/fetch-schemas";
import { fetchSchemas } from "@pipelines/generate-types/stages/fetch-schemas";
...
describe("fetchSchemas stage", () => {
	...
	it("tracks unresolved relations when target collection is unknown", async () => {
		...
		const result = await fetchSchemas(context, task);

		expect(task.output).toContain("1 relação(ões) não resolvida(s)");
		expect(
	...
	it("stores dependent collection names when includeDependents is enabled", async () => {
		...
		expect(
			Object.keys(result.pipelineContext?.collectionTypes ?? {}).sort(),
	...
	it("does not track unresolved relations when target is empty", async () => {
		...
		expect(task.output).toContain("0 relação(ões) não resolvida(s)");
		expect(
