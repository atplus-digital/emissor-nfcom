...
/**
 * Remove o prefixo técnico de tabela `t_` (ex.: `t_pessoas` → `pessoas`).
 */
export function stripTablePrefix(name: string): string {
	return name.replace(/^t_/, "");
}

/**
 * Remove o prefixo técnico de tabela/campo `t_`/`f_`
 * (ex.: `f_status` → `status`, `t_pessoas` → `pessoas`).
 */
export function stripTechnicalPrefix(name: string): string {
	return name.replace(/^[tf]_/, "");
}
...
function toPascalCase(name: string): string {
	const withoutPrefix = stripTablePrefix(name);
...
export function toCollectionConstantPrefix(collectionName: string): string {
	const withoutPrefix = stripTechnicalPrefix(collectionName.trim());
...
export function toFileName(collectionName: string): string {
	const withoutPrefix = stripTechnicalPrefix(collectionName);
	return withoutPrefix
		.split(/[^a-zA-Z0-9]+/)
		.filter(Boolean)
