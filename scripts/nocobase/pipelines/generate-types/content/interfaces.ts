...
import type { BaseInterfaceNamingConfig } from "@pipelines/generate-types/@types/script";
import {
	formatKey,
	stripTablePrefix,
	toBaseSchemaName,
...
function _toRelationSchemaName(collectionName: string): string {
	const cleanCollectionName = stripTablePrefix(collectionName).toLowerCase();
	if (!cleanCollectionName || cleanCollectionName === "t") {
		return toValidIdentifier(`${collectionName.toLowerCase()}RelationSchema`);
	}
	return toValidIdentifier(`${cleanCollectionName}RelationSchema`);
}
...
function _toMainSchemaName(collectionName: string): string {
	const cleanCollectionName = stripTablePrefix(collectionName).toLowerCase();
	if (!cleanCollectionName || cleanCollectionName === "t") {
		return toValidIdentifier(`${collectionName.toLowerCase()}Schema`);
	}
	return toValidIdentifier(`${cleanCollectionName}Schema`);
}
...
function _toCreateSchemaName(collectionName: string): string {
	const cleanCollectionName = stripTablePrefix(collectionName).toLowerCase();
	if (!cleanCollectionName || cleanCollectionName === "t") {
		return toValidIdentifier(`${collectionName.toLowerCase()}CreateSchema`);
	}
	return toValidIdentifier(`${cleanCollectionName}CreateSchema`);
}
...
function _toUpdateSchemaName(collectionName: string): string {
	const cleanCollectionName = stripTablePrefix(collectionName).toLowerCase();
	if (!cleanCollectionName || cleanCollectionName === "t") {
		return toValidIdentifier(`${collectionName.toLowerCase()}UpdateSchema`);
	}
	return toValidIdentifier(`${cleanCollectionName}UpdateSchema`);
}
