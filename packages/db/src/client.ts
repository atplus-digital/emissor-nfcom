 ...
 *
 * Conexão por `DATABASE_URL` (default `./data/emissor.db`, ADR-0005/0011). O pod roda
 ...
 *
 * Lê `DATABASE_URL` direto de `process.env` (ADR-0011): pacote de workspace não tem
 * acesso ao `#/env` do backend. O default espelha o zod do env do backend
 * (`apps/backend/src/env.ts`), que valida a variável no boot antes de qualquer
 * `getDb()` — exceção documentada à regra de nunca ler `process.env` no domínio.
 */
export function getDb(): CoordDB {
	if (_db) return _db;
	const url = process.env.DATABASE_URL ?? "./data/emissor.db";
