...
describe("asaas.client · criarCustomer", () => {
	it("POST /v3/customers com body JSON", async () => {
		const f = fakeFetch({
			status: 200,
			body: { id: "cus_new", name: "X", email: "x", cpfCnpj: "111" },
		});
		const client = createAsaasClient({ fetchImpl: f, ...NO_ENV_OPTS });
		const r = await client.criarCustomer({
			name: "X",
			email: "x",
			cpfCnpj: "111",
		});
...
describe("asaas.client · atualizarCustomer", () => {
	it("PUT /v3/customers/{id}", async () => {
		const f = fakeFetch({
			status: 200,
			body: { id: "cus_1", name: "Novo", email: "n", cpfCnpj: "111" },
		});
		const client = createAsaasClient({ fetchImpl: f, ...NO_ENV_OPTS });
...
describe("asaas.client · criarPayment", () => {
	it("POST /v3/payments", async () => {
		const f = fakeFetch({
			status: 200,
			body: { id: "pay_1", invoiceUrl: "https://asaas/1" },
		});
		const client = createAsaasClient({ fetchImpl: f, ...NO_ENV_OPTS });
...
describe("asaas.client · consultarPaymentPorExternalReference", () => {
	it("GET /v3/payments?externalReference=", async () => {
		const f = fakeFetch({
			status: 200,
			body: { data: [{ id: "pay_1", invoiceUrl: "u" }], totalCount: 1 },
		});
		const client = createAsaasClient({ fetchImpl: f, ...NO_ENV_OPTS });
...
describe("asaas.client · erros", () => {
	...
	it("m2: resposta não-JSON (HTML 503/WAF) → AsaasApiError com errors: [] e status preservado, não SyntaxError", async () => {
		const f = fakeFetch({
			status: 503,
		...
		try {
			...
			expect((e as AsaasApiError).status).toBe(503);
			expect((e as AsaasApiError).errors).toEqual([]);
		}
	});
});
